  ...
  return (
    <header
      className={`sticky top-0 z-50 border-b bg-white/95 backdrop-blur transition-shadow ${
        scrolled ? "border-ink-100 shadow-[0_2px_16px_rgba(11,21,26,0.08)]" : "border-transparent"
      }`}
    >
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between gap-3 px-4 sm:px-6 md:h-[72px]">
          ...
          >
            ...
            {dropOpen && (
              <div className="absolute start-0 top-full w-64 pt-2">
                <ul className="overflow-hidden rounded-2xl border border-ink-100 bg-white p-2 shadow-[0_16px_40px_rgba(11,21,26,0.12)]">
        ...
        <div className="hidden items-center gap-3 lg:flex">
          ...
          <Link
            href={`/${locale}/contact`}
            onClick={closeAll}
            className="rounded-full bg-brand-700 px-5 py-2.5 text-[15px] font-semibold text-white shadow-sm transition-all hover:-translate-y-px hover:bg-brand-800 hover:shadow"
          >
            {t.nav.cta}
          </Link>
        </div>
      ...
      {open && (
        <div className="max-h-[calc(100dvh-4rem)] overflow-y-auto border-t border-ink-100 bg-white lg:hidden">
          <nav
          ...
          >
            ...
            <div className="overflow-hidden rounded-xl border border-ink-100">
              ...
              {mobileServices && (
                <ul className="border-t border-ink-100 bg-ink-50/50 p-2">
                  {t.servicesIndex.map((s) => (
                    <li key={s.slug}>
                      <Link
                        href={`/${locale}/services/${s.slug}`}
                        onClick={closeAll}
                        className="block rounded-lg px-4 py-2.5 text-[15px] font-medium text-ink-700 transition-colors hover:bg-brand-50 hover:text-brand-800"
                      >
                        {s.name}
                      </Link>
                    </li>
            ...
            <Link
              href={`/${locale}/contact`}
              onClick={closeAll}
              className="mt-2 rounded-xl bg-brand-700 px-4 py-3.5 text-center text-base font-semibold text-white"
            >
              {t.nav.cta}
            </Link>
          </nav>
